...
namespace DocumentGenerator.Core.Services;
    ...
    public static class CreateDoc
    {
        ...
        public static void CreateAllDoc(string FilePathReference, string filePathIn, string foldelPathOut, string substrateFilePath)
        {
            dateTime = DateTime.Now;

            if (CreateDictionaryReference(out Dictionary<string, ReferenceMaterialDictionary> references, ref FilePathReference) &&
                ...
                CreateDictionaryCity(out Dictionary<string, string> cities, ref filePathIn))
            {
                ...
                CreateCityes(FilePathReference, filePathIn, foldelPathOut, players, cities, references, substrateFilePath);
                CreateModer(FilePathReference, filePathIn, foldelPathOut, players, cities, references, substrateFilePath);
        ...
        public static void CreateEnd(string folderPathIn, string foldelPathOut, Dictionary<string, ReferenceMaterialDictionary> referencesDic)
        {
            ...
            foreach (var kvp in referencesDic)
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    if (inputSheet != null)
                    {
                        ...
                        worksheet.Cells[count, 1].Value = $"{kvp.Value.TypeCompetition} «{kvp.Value.NameCompetition}», код {kvp.Key}";
                        ...
                        Console.WriteLine($"Прочитан файл {kvp.Value.TypeCompetition}, код {kvp.Key}. Всего участников посетило -> {countTyt - 2}");
                    }
                }
                else
                {
        ...
        public static void CreateCityes(string filePathReference, string filePathIn, string foldelPathOut,
            List<PlayersListStruct> playerList, Dictionary<string, string> citiesDic, Dictionary<string, ReferenceMaterialDictionary> referencesDic, string substrateFilePath)
        {
            ...
            foreach (var kvpair in citiesDic)
            {
                foreach (var currentSchool in schools[kvpair.Key])
                {
                    ...
                    worksheet.Cells["A1"].Value = "X Межрегиональный открытый фестиваль научно-технического творчества «РОБОАРТ-2024»";
                    ...
                    worksheet.Cells[$"C{row}"].Style.HorizontalAlignment = OfficeOpenXml.Style.ExcelHorizontalAlignment.Center;
                    worksheet.Cells[$"I{row}:J{row}"].Merge = true;
                    worksheet.Cells[$"I{row}"].Value = "Подпись";
                    worksheet.Cells[$"I{row}"].Style.HorizontalAlignment = OfficeOpenXml.Style.ExcelHorizontalAlignment.Center;
                    ...
                    var cityFileName = Path.Combine(foldelPathOut, "Города-дистант", $"{kvpair.Key.Replace(@"\", "").Replace("\"", "")} {currentSchool.Replace(@"\", "").Replace("\"", "")}.xlsx");
        ...
        public static void CreateModer(string filePathReference, string filePathIn, string foldelPathOut,
            List<PlayersListStruct> playerList, Dictionary<string, string> citiesDic, Dictionary<string, ReferenceMaterialDictionary> referencesDic, string substrateFilePath)
        {
            int k = 0;
            ...
            foreach (var kvpair in referencesDic)
            {
                ...
                worksheet.Cells["A3"].Value = $"{kvpair.Value.TypeCompetition} {kvpair.Value.NameCompetition}, код {kvpair.Key}";
                worksheet.Cells["A3"].Style.HorizontalAlignment = OfficeOpenXml.Style.ExcelHorizontalAlignment.Center;
                ...
                var moderFileName = Path.Combine(foldelPathOut, "модерам-дистант", $"{kvpair.Key.Replace(@"\", "").Replace("\"", "")}.xlsx");
                File.WriteAllBytes(moderFileName, package.GetAsByteArray());

                Console.WriteLine($"Создан файл: {kvpair.Value.TypeCompetition} {kvpair.Value.NameCompetition} -> {row - 6} участников");
                k += row - 6;
            }
        ...
        public static bool CreateDictionaryCity(out Dictionary<string, string> dictionary, ref string filePath)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"Успешное чтение файла с городами: {filePath} за {(DateTime.Now - dateTime).TotalSeconds:F2} сек.");
                return true;
            }
            catch (Exception e)
            {
        ...
        public static bool CreateDictionaryReference(out Dictionary<string, ReferenceMaterialDictionary> dictionary, ref string filePath)
        {
            dictionary = null;
                ...
                for (int i = 2; i <= excelTable.Dimension.Rows; i++)
                {
                    if (excelTable.Cells[i, 4]?.Value != null)
                    {
                        list.Add(new PlayersListStruct
                        {
                            ...
                            BirthdayPlayers = excelTable.Cells[i, 5]?.Value != null ? Convert.ToDateTime(excelTable.Cells[i, 5].Value) : default(DateTime),
